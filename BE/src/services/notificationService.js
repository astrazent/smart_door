...
const createNotificationService = async payload => {
    if (!payload.title || !payload.message) {
        throw new ApiError(StatusCodes.BAD_REQUEST, 'title và message là bắt buộc')
    }
...
const deleteNotificationService = async id => {
    ...
    if (!deleted) {
        throw new ApiError(StatusCodes.NOT_FOUND, 'Không tìm thấy thông báo để xóa')
    }
    return { message: 'Xóa thông báo thành công' }
}

const listNotificationsService = async (limit = 50, offset = 0) => {
    const notifications = await NotificationModel.listNotifications(limit, offset)
    return notifications
}

const listNotificationsByUserService = async (user_id, limit = 50, offset = 0) => {
    const notifications = await NotificationModel.listNotifications(limit, offset)
